...
/*   Updated: 2023/04/19 23:55:04 by seykim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*res[O_MAX];
	char		*arr;
	int			x;

	if (fd < 0 || fd > O_MAX || BUFFER_SIZE <= 0)
		return (0);
	if (!res[fd])
		res[fd] = ft_strdup("");
	if (!res[fd])
		return (ft_free(&res[fd]));
	arr = (char *)malloc(sizeof(char) * (BUFFER_SIZE + 1));
	if (!arr)
		return (ft_free(&res[fd]));
	x = read(fd, arr, BUFFER_SIZE);
	if (!(read_line(fd, x, &arr, &res[fd])))
		return (0);
	free(arr);
	if (x < 0)
		return (ft_free(&res[fd]));
	return (print_arr(&res[fd]));
}
